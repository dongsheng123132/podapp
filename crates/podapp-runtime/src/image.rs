...
//! 都交给 ffmpeg（宿主的工具目录里通常已经有）。没有 ffmpeg 时给明确提示，
//! 而不是悄悄出一张错图。
//!
//! **独立可插拔**：纯 std + serde_json + flate2。删它：lib.rs 去 `mod image;`，
//! miniapp.rs 去 dispatch 里 "image." 那一支。

// 像素循环里 `for c in 0..3 { ch[c].push(...) }` 这类写法，clippy 会建议改成迭代器。
// 这份图像实现是从已发版代码原样搬过来的：改写下标循环收益为零，而下标错位的风险是实打实的 ——
// 通道串位不会 panic，只会让颜色悄悄不对，而「颜色有点怪」是最难被报成 bug 的一类问题。
...
/// 按句柄取一份像素拷贝。**第三方能力提供方的入口。**
///
/// 没有它，任何图像类能力（扫码、识别、测量）都写不了 —— 只能把整张图经桥来回搬，
/// 而桥上的 `pixels` 卡在 100 万像素以内，整图必被拒。
/// 这正是[能力注册表](crate::capability)要能被外部用起来的前提：
/// 开放的扩展点必须真的够得着数据，否则「可插拔」只是个说法。
///
/// 给的是**拷贝**不是引用：句柄背后的图会被 `fillRect` 这类动词就地改写，
/// 借出引用等于把并发问题甩给调用方。
pub fn get_img(id: &str) -> Result<Img, String> {
    get(id)
}

/// 把一张图放进句柄表，返回新句柄。能力提供方产出图像时用它。
pub fn put_img(img: Img) -> String {
    put(img)
}
...
fn ffmpeg() -> Option<PathBuf> {
    ...
    for base in [crate::home(), PathBuf::from(&home).join(".uking")] {
        for n in names {
            cands.push(base.join("tools").join("ffmpeg").join(n));
            cands.push(base.join("tools").join(n));
        }
    }
...
/// `pod.image.<verb>(...)`。args 是位置参数数组（桥那边用 Proxy 透传的）。

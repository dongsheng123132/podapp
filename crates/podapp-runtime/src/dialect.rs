...
//! `tests/roundtrip.rs` 拿三个真实清单跑 uking→内部模型→podapp→内部模型，
...
impl Dialect {
    ...
    pub fn profile_const(self) -> &'static str {
        match self {
            ...
            Dialect::UKing => "action-parity/miniapp@0.1",
        }
    }
...
mod tests {
    ...
    fn profile_constants_are_distinct() {
        assert_ne!(Dialect::PodApp.profile_const(), Dialect::UKing.profile_const());
        assert_ne!(Dialect::PodApp.manifest_file(), Dialect::UKing.manifest_file());
        assert_ne!(Dialect::PodApp.pkg_ext(), Dialect::UKing.pkg_ext());
    }

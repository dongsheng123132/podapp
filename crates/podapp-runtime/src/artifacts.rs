...
//! GUI 侧则用 <scheme>://localhost/artifact/<id> 取像素。两条路产出到同一个地方。
//!
...
/// 收件箱所在的家目录。`<PREFIX>_ARTIFACTS_ROOT` 能顶掉它 ——
/// 自检靠这个保证绝不往用户真实的收件箱里塞测试产物。
fn artifacts_home() -> PathBuf {
    if let Ok(p) = std::env::var(format!("{}_ARTIFACTS_ROOT", crate::profile().env_prefix)) {
